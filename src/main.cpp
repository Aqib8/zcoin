...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << startBits << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp)); 
//<< extraNonce;
        ...
        block.nVersion = 1;
        block.nTime    = 1537418291; // 10/31/2014 @ 5:24pm (UTC)
...
if (true && block.GetHash() != hashGenesisBlock)
...
#endif
                if (thash <= hashTarget)		//**Loop: if foud the hash, break
                    break;
        ...
        assert(block.hashMerkleRoot == uint256("0xfbf02d1cbf20366162160715557bd7b7604a6f0d286a5432ba1292a39f8f2bf6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xea, 0xdb, 0xff, 0xfd };
